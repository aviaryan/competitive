...
main(){
	...
	for(t2=1; t2<=n; t2++){
		...
		plzgo = gowh ? 0 : 1; // allow odds like 5
		// plzgo = 1;
		lstfrom = 4;

		printf("\n\tstart of new %d - plzgo %d", t2, plzgo);
		printStatus(sk[1]);
		printStatus(sk[2]);
		printStatus(sk[3]);
		for(ct=1; ct<=mids; ct++){ //ct=2 hour of action
			...
			// bto c and c to a priority
			else if (s[2]<s[3] && lstto != 2 && (lstfrom != 3 || s[3] == n+1 || 1) ) // b to c - holder for odd = b
				sk[2][s[2]] = 0, sk[3][s[2]] = 1, lstto = 3, lstfrom = 2, printprg(lstto, lstfrom);
			else if (s[3]<s[1] && lstto != 3 && (lstfrom != 1 || s[1] == n+1 || 1) ) // c to a
				...
				printf("this should not show %d - %d - %d - %d - %d", s[1], s[2], s[3], lstto, lstfrom), done = 0, lstfrom = 5;

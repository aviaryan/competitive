...
ll optGrid(int pre_choose){
	...
	if (mxH2 - mxH1 > 1){
		...
		if (mxH1 != 0 && mxH2 != n && (mxH2 - mxH1) > 2){
			t1 = *(find(hzCuts.begin(), hzCuts.end(), mxH1) - 1);
			t2 = *(find(hzCuts.begin(), hzCuts.end(), mxH2) + 1);
			t_min = max(mx_sum(mxH1+1, mxH2-1, mxV1, mxV2), max_grid_vert(mxH2-1, t2));
			t_min = max(t_min, max_grid_vert(t1, mxH1+1));
			if (t_min < mn) type = 5; mn = t_min;
			m_min[5] = t_min;
		}
	}
	if (mxV2 - mxV1 > 1){
		...
		if (mxV1 != 0 && mxV2 != n && (mxV2 - mxV1) > 2){
			t1 = *(find(vtCuts.begin(), vtCuts.end(), mxV1) - 1);
			t2 = *(find(vtCuts.begin(), vtCuts.end(), mxV2) + 1);
			t_min = max(mx_sum(mxH1, mxH2, mxV1+1, mxV2-1), max_grid_horz(mxV2-1, t2));
			t_min = max(t_min, max_grid_horz(t1, mxV1+1));
			if (t_min < mn) type = 6; mn = t_min;
			m_min[6] = t_min;
		}
	}
	// madmode
	if (pre_choose != 0 && type == 0){
		type = pre_choose;
		mn = m_min[type];
	}
	// set value
...
int main(){
	...
	// when grid done, get the max cell, then change grid
	ll st;
	for (i=0; i<150; i++){
		st = optGrid(0);
		if (st == 0) break;
	}
	// mad mode (all directions)
	vector <int> hc, vc, hc1, vc1, hc2, vc2, hc3, vc3;
	ll ost;
	for (i=0; i<5; i++){
		hc = hzCuts; vc = vtCuts;
		ost = optGrid(0);
		...
		for (j=1; j<=6; j++){
			hc1 = hzCuts; vc1 = vtCuts;
			st = optGrid(j);
			if (st < ost) break;

			for (k=1; k<=6; k++){
				hc2 = hzCuts; vc2 = vtCuts;
				st = optGrid(k);
				if (st < ost) break;

				for (k2=1; k2<=6; k2++){
					hc3 = hzCuts; vc3 = vtCuts;
					st = optGrid(k2);
					if (st < ost) break;
					hzCuts = hc3; vtCuts = vc3;
				}

				if (st < ost) break;
				hzCuts = hc2; vtCuts = vc2;
			}

			if (st < ost) break;
			hzCuts = hc1; vtCuts = vc1;
		}

		if (st >= ost){
			hzCuts = hc; vtCuts = vc;
			break; // no need of this
		}
	}
